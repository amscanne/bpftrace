...
# These headers copied manually. They are not normally part of the userspace
# API, but when building BPF code sometimes it is useful to use `__KERNEL__`.
declare -a MANUAL_HEADERS
MANUAL_HEADERS=(
  linux/compiler.h
  linux/compiler_types.h
  linux/compiler_attributes.h
  linux/kasan-checks.h
  linux/kcsan-checks.h
  # We only care about `compiler-clang` for obvious reasons.
  linux/compiler-clang.h
)
# This is the list of architectures that we support. When we hit an
# arch-specific header, we put it in a special place and attempt to copy all
# other arch-specific headers. Note that if a header exists only on one
# architecture, it should be added above and the failure ignored below.
declare -a ARCHES
ARCHES=(
  x86
  arm
...
# These headers is not part of the user API, but is still used
for header in "${MANUAL_HEADERS[@]}"; do
  basepath=$(dirname "include/${header}")
  cp --preserve=mode "${LINUX}/include/${header}" "src/stdlib/${basepath}"
done

# Finally, this one is really manual. Copy `rwonce.h` into each of the
# architecture directories.
for arch in "${ARCHES[@]}"; do
  cp --preserve=mode "${LINUX}/include/asm-generic/rwonce.h" "src/stdlib/include/asm/${arch}"
done
