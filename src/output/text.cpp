...
template <typename T>
static void try_add(Primitive &p)
{
  if (std::holds_alternative<T>(p.variant)) {
    auto v = std::get<T>(p.variant);
...
template <typename T>
static void try_sub(Primitive &p)
{
  if (std::holds_alternative<T>(p.variant)) {
    auto v = std::get<T>(p.variant);
...
struct TextEmitter<Value::TimeSeries> {
  static void emit(std::ostream &out, const Value::TimeSeries &ts)
  {
    ...
    if (min_value == max_value) {
      ...
      try_add<uint64_t>(max_value);
      try_add<int64_t>(max_value);
      try_sub<uint64_t>(min_value);
      try_sub<int64_t>(min_value);
    }
