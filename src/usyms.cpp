...
{
  auto cache_type = config_.must_get<ConfigKey::user_symbol_cache_type>();
...
struct blaze_symbolizer *Usyms::create_symbolizer() const
{
  blaze_symbolizer_opts opts = {
    ...
    .demangle = config_.must_get<ConfigKey::cpp_demangle>(),
  };
...
{
#ifdef HAVE_BLAZESYM
  if (config_.must_get<ConfigKey::use_blazesym>()) {
                               ...
                               bool show_module)
{
  ...
  if (psyms && bcc_symcache_resolve(psyms, addr, &usym) == 0) {
    ...
    if (config_.must_get<ConfigKey::cpp_demangle>())
...
struct bcc_symbol_option &Usyms::get_symbol_opts()
{
  static struct bcc_symbol_option symopts = {
    ...
    .lazy_symbolize = config_.must_get<ConfigKey::lazy_symbolication>() ? 1 : 0,
