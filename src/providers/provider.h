...
public:
  ...
  virtual Result<btf::Types> get_kernel_btf(
      [[maybe_unused]] const std::string &module = "vmlinux") const
  {
    return make_error<SystemError>("No BTF lookup available", ENOSYS);
  }

  // Lists all available modules for kernel BTF.
  virtual Result<std::vector<std::string>> get_kernel_modules() const
  {
    return make_error<SystemError>("No BTF modules available", ENOSYS);
  }
  ...
  virtual Result<btf::Types> get_user_btf(
      [[maybe_unused]] const std::string &binary_path) const
  {
    return make_error<SystemError>("No BTF lookup available", ENOSYS);
  }
};
