...
//   double dispatch is that we can control the passed arguments and return
//   value which is hard to do with double dispatch, a different Accept() would
//   have to be implement for every new signature needed. Which makes tree
//   modification harder to implement.
//
//   The downside is that this way of dispatching is significantly slower.
//
//   The core is a map of Types (using std::type_index) to FunctionPointers. A
//   bit of templating makes it reusable for different types of arguments and
//   return types.
//
//   The current implementation only supports dispatching on Nodes and only
//   support one argument, a Visitor. But this can by extended with some
//   template magic.
//
//   \code
