...
public:
  explicit StructType(ASTContext &ctx, Location &&loc, std::string name)
      : Node(ctx, std::move(loc)), name(std::move(name)) {};
  explicit StructType(ASTContext &ctx, const Location &loc, const StructType &other)
      : Node(ctx, loc + other.loc), name(other.name) {};

  bool operator==(const StructType &other) const { return name == other.name; }
  std::strong_ordering operator<=>(const StructType &other) const { return name <=> other.name; }
...
public:
  explicit UnionType(ASTContext &ctx, Location &&loc, std::string name)
      : Node(ctx, std::move(loc)), name(std::move(name)) {};
  explicit UnionType(ASTContext &ctx, const Location &loc, const UnionType &other)
      : Node(ctx, loc + other.loc), name(other.name) {};

  bool operator==(const UnionType &other) const { return name == other.name; }
  std::strong_ordering operator<=>(const UnionType &other) const { return name <=> other.name; }
...
public:
  explicit CastOrBinop(ASTContext &ctx,
                       ...
                       Operator ambiguous_op,
                       Expression rhs)
      : Node(ctx, std::move(loc)),
        paren_expr(std::move(paren_expr)),
        ambiguous_op(ambiguous_op),
        rhs(std::move(rhs)) {};
  ...
  explicit CastOrBinop(ASTContext &ctx,
      ...
      : Node(ctx, loc + other.loc),
        paren_expr(clone(ctx, loc, other.paren_expr)),
        ambiguous_op(other.ambiguous_op),
  ...
  {
    return paren_expr == other.paren_expr &&
           ambiguous_op == other.ambiguous_op &&
           rhs == other.rhs;
  }
  ...
  {
    if (auto cmp = paren_expr <=> other.paren_expr; cmp != 0)
      return cmp;
    if (auto cmp = ambiguous_op <=> other.ambiguous_op; cmp != 0)
      return cmp;
    return rhs <=> other.rhs;
  }

  // Expression in parentheses - could be a type or an expression
  Expression paren_expr;

  // Ambiguous operator: MUL, PLUS, MINUS, or BAND
  Operator ambiguous_op;
