...
public:
  ...
  // Furthermore, the expansion will only work with `ErrorOr<T>` or
  // `ErrorOrSuccess` types, so the lambda will need to return one of those.
  // Otherwise, the first argument must be the ASTContext&, but all other
  // arguments will be automatically plumbed.
      ...
      std::function<ErrorOr<Return>(ASTContext &, Inputs &...)> fn)
  {
    ...
    return Pass(name,
                std::move(inputs),
                std::move(outputs),
                [fn](ASTContext &ast, PassContext &ctx) -> ErrorOrSuccess {
                  ...
                  auto [val, warnings] = result.unwrap();
                  if constexpr (std::is_same_v<Return, std::monostate>) {
                     ...
                     std::function<Return(Inputs...)> fn)
  {
    ...
    if constexpr (std::is_same_v<Return, void>) {
      return create(name, [fn](Inputs... inputs) -> ErrorOrSuccess {
        fn(inputs...);
        return Success();
      });
    } else {
  ...
  ErrorOrSuccess run(ASTContext &ast, PassContext &ctx) const
  {
    return fn_(ast, ctx);
  }
...
private:
  ...
  using Fn = std::function<ErrorOrSuccess(ASTContext &ast, PassContext &ctx)>;
...
public:
  ...
  ErrorOrSuccess foreach(std::function<ErrorOrSuccess(const Pass &)> fn);

  // put adds a pass which always pushes the given static data.
  template <typename T>
  void put(T &t)
  {
    add(Pass::create(T::type_name(), [&t]([[maybe_unused]] ASTContext &ast) { return std::ref(t); }));
  }
