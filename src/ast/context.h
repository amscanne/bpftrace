...
public:
  ...
  // Finds and removes comments that should be associated with this
  // node, and remove them from the object.
  std::vector<Variant> pop_until(const SourceLocation::Position &pos);

private:
  using InternalMap =
      std::map<SourceLocation, std::variant<std::monostate, size_t>>;
...
public:
  ...
  void add_comment(SourceLocation loc)
  {
    ...
    state_->metadata_.emplace(std::move(loc), std::monostate{});
  }

  void add_vspace(SourceLocation loc, size_t elems)
  {
    // Vspace is stored as a simple unsigned value.
    state_->metadata_.emplace(std::move(loc), elems);
  }
