...
SizedType Expression::type() const
{
  return std::visit(
      [](const auto &v) -> SizedType {
...
SizedType StackMode::type() const
{
  auto stack_mode = bpftrace::Config::get_stack_mode(mode);
  if (stack_mode.has_value()) {
    auto type = CreateStackMode();
    type.stack_type.mode = stack_mode.value();
    return type;
  } else {
    addError() << "Unknown stack mode: '" + mode + "'";
    return CreateNone();
  }
}
...
std::string Probe::name() const
{
  ...
  std::ranges::transform(attach_points.begin(),
                         attach_points.end(),
...
{
  return std::ranges::any_of(attach_points,
                             [probe_type](const AttachPoint &ap) {
