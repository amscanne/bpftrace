...
std::vector<MetadataIndex::Variant> MetadataIndex::pop_until(
    const SourceLocation::Position &pos)
{
  ...
  while (it != map_.end()) {
    ...
    if (std::holds_alternative<size_t>(it->second)) {
      ...
      result.emplace_back(std::get<size_t>(it->second));
    } else {
      // Load the comment from the serialized source.
      result.emplace_back(source_->read(it->first));
    }
    it = map_.erase(it);
  }
