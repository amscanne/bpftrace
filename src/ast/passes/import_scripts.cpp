...
static void import_ast(ASTContext &ast, const Location &loc, const ASTContext &other)
{
  ...
  if (other.root) {
    for (const auto &stmt : other.root->c_statements) {
      ast.root->c_statements.push_back(clone(ast, loc, stmt));
    }
    for (const auto &decl : other.root->map_decls) {
      ast.root->map_decls.push_back(clone(ast, loc, decl));
    }
    for (const auto &fn : other.root->functions) {
      ast.root->functions.push_back(clone(ast, loc, fn));
    }
    for (const auto &macro : other.root->macros) {
      ast.root->macros.push_back(clone(ast, loc, macro));
    }
    for (const auto &probe : other.root->probes) {
      ast.root->probes.push_back(clone(ast, loc, probe));
    }
  }
}
  ...
  return Pass::create("ImportExternalScripts",
                      [](ASTContext &ast, Imports &imports) {
                        for (const auto &[name, obj] : imports.scripts) {
                          if (!obj.internal) {
                            import_ast(ast, obj.node.loc, obj.ast);
                          }
                        }
                      });
}
