...
ScopedExpr CodegenLLVM::visit(Call &call)
{
  if (call.func == "count") {
    ...
    Map &map = *call.map;
    ScopedExpr scoped_del = visit(*call.vargs.front());
...
ScopedExpr CodegenLLVM::visit(FieldAccess &acc)
{
  ...
  if (inBpfMemory(type)) {
    return readDatastructElemFromStack(scoped_arg.value(),
  ...
  if (var.type.IsArrayTy() || var.type.IsRecordTy()) {
    ...
    return ScopedExpr(
        b_.CreateStore(b_.CreatePtrToInt(scoped_expr.value(), b_.getInt64Ty()),
                       getVariable(var.ident).value), std::move(scoped_expr));
...
ScopedExpr CodegenLLVM::readDatastructElemFromStack(Value *src_data,
                                                    Value *index,
                                                    llvm::Type *data_type,
...
ScopedExpr CodegenLLVM::probereadDatastructElem(Value *src_data,
                                                Value *offset,
                                                const SizedType &data_type,
                                                const SizedType &elem_type,
                                                ScopedExpr &&scoped_del,
