...
ScopedExpr CodegenLLVM::visit(Call &call)
{
  if (call.func == "count") {
    ...
    auto scoped_arg2 = visit(call.vargs.at(1));
    ...
    Map &map = *call.map;
    auto scoped_del = visit(*call.vargs.front());
    ...
    if (call.vargs.size() > 1) {
      auto scoped_arg = accept(call.vargs.at(1));
    ...
    if (!inside_subprog_ &&
      ...
      return ScopedExpr(b_.CreateSeqPrintf(ctx_,
    ...
    if (std::holds_alternative<Map*>(call.vargs.at(0))) {
      auto &arg = call.vargs.at(0);
      auto &map = **std::get_if<Map*>(&arg);
      if (map.key_expr)
    ...
    if (map_has_single_elem(map.type, map.key_type)) {
      ...
      return ScopedExpr(
          _.CreateForEachMapElem(ctx_, map, map_len_func_, nullptr, call.loc));
    }
    ...
    auto scoped_arg = accept(&arg);
    Value *sig_number = b_.CreateIntCast(scoped_arg.value(),
                                         b_.getInt32Ty(),
                                         arg.type.IsSigned());
    b_.CreateSignal(ctx_, sig_number_, call.loc);
    return ScopedExpr();
    ...
    Value *len = b_.CreateIntCast(arg_len.value(), b_.getInt64Ty(), false);
...
ScopedExpr CodegenLLVM::binop_integer_array(Binop &binop)
{
  ...
  auto scoped_right = accept(binop.right);
...
void CodegenLLVM::binop_ptr(Binop &binop)
{
  ...
  auto scoped_left = accept(binop.left);
  auto scoped_right = accept(binop.right);
  Value *lhs = scoped_left.value();
  Value *rhs = scoped_right.value();
...
static bool unop_skip_accept(Unop &unop)
{
  if (unop.expr->type.IsIntTy()) {
    if (unop.op == Operator::INCREMENT || unop.op == Operator::DECREMENT)
      return std::holds_alternative<Map*, Variable*>(unop.expr);
  }

  return false;
}

ScopedExpr CodegenLLVM::unop_int(Unop &unop, ScopedExpr &&expr)
{
  SizedType &type = unop.expr->type;
  switch (unop.op) {
    case Operator::LNOT: {
      auto ty = expr.value()->getType();
      Value *zero_value = Constant::getNullValue(ty);
      Value *expr = b_.CreateICmpEQ(expr.value(), zero_value);
      // CreateICmpEQ() returns 1-bit integer
      // Cast it to the same type of the operand
      // Use unsigned extension, otherwise !0 becomes -1
      return ScopedExpr(b_.CreateIntCast(expr.value(), ty, false));
    }
    case Operator::BNOT:
      return ScopedExpr(b_.CreateNot(expr.value()));
    case Operator::MINUS:
      return ScopedExpr(b_.CreateNeg(expr.value()));
    case Operator::INCREMENT:
    case Operator::DECREMENT: {
      return createIncDec(unop, std::move(expr));
    }
    case Operator::MUL: {
      ...
      b_.CreateProbeRead(ctx_, dst, type, expr.value(), unop.loc);
...
ScopedExpr CodegenLLVM::unop_ptr(Unop &unop, ScopedExpr &&expr)
{
  SizedType &type = unop.expr->type;
  switch (unop.op) {
    case Operator::MUL: {
      if (unop.type.IsIntegerTy() || unop.type.IsPtrTy()) {
        ...
        b_.CreateProbeRead(
            ctx_, dst, *et, expr.value(), unop.loc, type.GetAS());
        Value *value = b_.CreateLoad(b_.GetType(*et), dst);
        b_.CreateLifetimeEnd(dst);
        return ScopedExpr(value); // expr is consumed; no longer needed.
      }
    ...
    default:
      return std::move(expr); // Do nothing.
  }
}

ScopedExpr CodegenLLVM::visit(Unop &unop)
{
  auto scoped_expr = visit(unop.expr);
  SizedType &type = unop.expr->type;
  if (type.IsIntegerTy()) {
    return unop_int(unop, std::move(scoped_expr));
  } else if (type.IsPtrTy() || type.IsCtxAccess()) // allow dereferencing args
  {
    return unop_ptr(unop, std::move(scoped_expr));
  } else {
...
ScopedExpr CodegenLLVM::visit(Ternary &ternary)
{
  ...
  auto scoped_expr = accept(ternary.cond);
...
ScopedExpr CodegenLLVM::visit(FieldAccess &acc)
{
...
#endif
      return readDatastructElemFromStack(args,
  ...
  if (inBpfMemory(type)) {
    readDatastructElemFromStack(scoped_arg.value(),
    ...
    if (field.type.IsIntTy() &&
      ...
      return probereadDatastructElem(scoped_arg.value(),
                                     b_.getInt64(field.offset),
                                     type,
                                     field.type,
                                     std::move(scoped_arg),
...
ScopedExpr CodegenLLVM::visit(ArrayAccess &arr)
{
  ...
  if (inBpfMemory(type))
    readDatastructElemFromStack(
        array, scoped_index.value(), type, elem_type, std::move(scoped_expr));
  else {
    ...
    return probereadDatastructElem(array,
                                   offset,
                                   type,
                                   elem_type,
                                   std::move(scoped_expr),
...
{
  auto scoped_expr = accept(cast.expr);
...
{
  auto scoped_del = visit(expr.expr);
}
...
ScopedExpr CodegenLLVM::visit(AssignMapStatement &assignment)
{
  ...
  b_.CreateMapUpdateElem(
      ctx_, map.ident, scoped_key.value(), val, assignment.loc);
...
ScopedExpr CodegenLLVM::getMapKey(Map &map, Expression *key_expr)
{
  ...
  return ScopedExpr(key, [this, key, alloca_created_here]() {
                                   ...
                                   const location &loc)
{
  ...
  if (inBpfMemory(map.key_expr->type)) {
    ...
    if (map.key_expr->type.IsArrayTy() || map.key_expr->type.IsRecordTy()) {
      ...
      b_.CreateProbeRead(
          ctx_, offset_val, map.key_expr->type, scoped_expr.value(), expr->loc);
      if ((map_key_size % 8) != 0)
        aligned = false;
    } else {
                                         ...
                                         AsyncAction async_action)
{
  ...
  for (size_t i = 1; i < call.vargs.size(); i++) {
    ...
    if (needMemcpy(arg.type))
      ...
      b_.CreateStore(b_.CreateIntCast(scoped_arg.value() _,
                                      b_.getInt64Ty(),
                                      arg.type.IsSigned()),
                     offset);
    else
      b_.CreateStore(scoped_arg.value() _, offset);
  }
                                              ...
                                              ScopedExpr &&scoped_del)
{
  ...
  if (elem_type.IsIntegerTy() || elem_type.IsPtrTy()) {
    ...
    return b_.CreateDatastructElemLoad(elem_type, src, true, elem_type.GetAS());
  } else {
                                              ...
                                              ScopedExprDeleter &&scoped_del)
{
  return readDatastructElemFromStack(
      src_data, index, b_.GetType(data_type), elem_type, std::move(scoped_del));
}
                                          ...
                                          const std::string &temp_name)
{
  ...
  if (elem_type.IsRecordTy() || elem_type.IsArrayTy()) {
    ...
    return ScopedExpr(src, std::move(scoped_del));
...
ScopedExpr CodegenLLVM::createIncDec(Unop &unop, ScopedExpr &&expr)
{
