...
void CodegenLLVM::visit(Builtin &builtin)
{
  if (builtin.ident == "nsecs") {
    ...
    if (builtin.type.is_funcarg) {
      expr_ = b_.CreatKFuncArg(getContext(), builtin.type, builtin.ident);
      return;
    }
...
{
  assert(src_data->getType()->isPointerTy());
                                          ...
                                          const std::string &temp_name)
{
  ...
  // GEP to support the appropriate `preserve_static_offsets` intrinsic.
  auto raw_ptr = b_.getInt8Ty()->getPointerTo();
  if (src_data->getType() != raw_ptr)
    src_data = b_.CreatePointerCast(src_data, raw_ptr);
  Value *src = b_.CreateGEP(b_.getInt8Ty(), src_data, offset);
  auto dst_type = b_.GetType(elem_type);
  if (dst_type->getPointerTo() != raw_ptr)
    src = b_.CreatePointerCast(src, dst_type->getPointerTo());
