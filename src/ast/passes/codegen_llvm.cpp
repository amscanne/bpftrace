      ...
      debug_(*module_)
{
  ...
  const std::string license = bpftrace_.config_->must_get<ConfigKey::license>();
...
ScopedExpr CodegenLLVM::visit(Call &call)
{
  if (call.func == "count") {
    ...
    auto max_strlen = bpftrace_.config_->must_get<ConfigKey::max_strlen>();
    ...
    b_.CreateMemsetBPF(buf,
                       b_.getInt8(0),
                       bpftrace_.config_->must_get<ConfigKey::max_strlen>());
    const auto max_size = bpftrace_.config_->must_get<ConfigKey::max_strlen>();
    Value *sz;
    if (call.vargs.size() > 1) {
      auto scoped_arg = visit(call.vargs.at(1));
...
ScopedExpr CodegenLLVM::visit(Probe &probe)
{
  ...
  auto max_bpf_progs = bpftrace_.config_->must_get<ConfigKey::max_bpf_progs>();
                                ...
                                const CodegenResources &codegen_resources)
{
  ...
  if (bpftrace_.feature_->has_map_ringbuf()) {
    auto entries = bpftrace_.config_->must_get<ConfigKey::perf_rb_pages>() *
                   4096;
