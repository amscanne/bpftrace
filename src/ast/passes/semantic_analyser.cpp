                                           ...
                                           const Expression *expr)
{
  if (const auto *map = dynamic_cast<const Map *>(expr)) {
    ...
    if (map->type.IsMultiOutputMapTy())
      return false;
...
void SemanticAnalyser::visit(String &string)
{
  ...
  auto str_len = bpftrace_.config_->must_get<ConfigKey::max_strlen>();
...
void SemanticAnalyser::visit(Builtin &builtin)
{
  if (builtin.ident == "ctx") {
    ...
    builtin.type = CreateStack(
        true,
        StackType{
            .mode = bpftrace_.config_->must_get<ConfigKey::stack_mode>() });
...
void SemanticAnalyser::visit(Call &call)
{
  ...
  if (call.func == "hist") {
    ...
    if (check_varargs(call, 1, 2)) {
      ...
      auto strlen = bpftrace_.config_->must_get<ConfigKey::max_strlen>();
...
void SemanticAnalyser::check_stack_call(Call &call, bool kernel)
{
  ...
  stack_type.mode = bpftrace_.config_->must_get<ConfigKey::stack_mode>();
  ...
  switch (call.vargs.size()) {
    ...
    case 1: {
      if (auto *ident = dynamic_cast<Identifier *>(call.vargs.at(0))) {
        ConfigParser<ConfigKey::stack_mode> parser;
        auto ok = parser.parse(stack_type.mode, ident->ident);
        if (ok) {
          stack_type.mode = *ok;
        } else {
...
{
  if (!bpftrace_.config_->must_get<ConfigKey::unstable_map_decl>()) {
...
void SemanticAnalyser::visit(AttachPoint &ap)
{
  if (ap.provider == "kprobe" || ap.provider == "kretprobe") {
    ...
    if (is_final_pass()) {
      ...
      if (bpftrace_.config_->must_get<ConfigKey::missing_probes>() !=
