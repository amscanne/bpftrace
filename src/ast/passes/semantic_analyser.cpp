...
bool SemanticAnalyser::is_valid_assignment(const Expression &expr)
{
  ...
  if (expr.type().IsMultiOutputMapTy() || expr.type().IsCastableMapTy()) {
    return false;
...
void SemanticAnalyser::visit(Call &call)
{
  ...
  if (call.func == "hist") {
    ...
    if (check_varargs(call, 1, 3)) {
      ...
      } else if (arg.type().IsMultiOutputMapTy()) {
        call.addError()
            << "Map type " << arg.type()
  ...
  if (other.IsPtrTy()) {
    if (compare) {
      ...
      if (is_final_pass()) {
        ...
        if (*le != *re) {
          binop.addWarning() << "comparison of distinct pointer types ('" << *le
                             << ", '" << *re << "')";
        }
      }
    } else if (logical) {
...
void SemanticAnalyser::visit(Unop &unop)
{
  if (unop.op == Operator::INCREMENT || unop.op == Operator::DECREMENT) {
    ...
    auto *acc = unop.expr.as<MapAccess>();
    auto *var = unop.expr.as<Variable>();
    if (!acc && !var) {
      unop.addError() << "The " << opstr(unop)
                      << " operator must be applied to a map or variable";
    }

    if (acc && acc->map->type().IsNoneTy()) {
