                               ...
                               int usdt_location_idx)
{
  ...
  probe.log_size = config_->must_get<ConfigKey::log_size>();
...
void perf_event_printer(void *cb_cookie, void *data, int size)
{
  ...
  out:
    ...
    util::cat_file(fmt.format_str(arg_values).c_str(),
                   bpftrace->config_->must_get<ConfigKey::max_cat_bytes>(),
                   buf);
    ...
    switch (arg.type.GetTy()) {
      ...
      case Type::string: {
        ...
        arg_values.push_back(std::make_unique<PrintableString>(
            ...
            config_->must_get<ConfigKey::max_strlen>(),
            config_->must_get<ConfigKey::str_trunc_trailer>().c_str()));
        break;
      }
      case Type::buffer: {
        auto length =
...
int BPFtrace::prerun() const
{
  ...
  auto max_probes = config_->must_get<ConfigKey::max_probes>();
  if (num_probes == 0) {
    if (!bt_quiet)
...
int BPFtrace::setup_perf_events()
{
  ...
  for (int cpu : cpus) {
    void *reader = bpf_open_perf_buffer(
        ...
        -1,
        cpu,
        config_->must_get<ConfigKey::perf_rb_pages>());
    if (reader == nullptr) {
                                        ...
                                        uint64_t nsecs)
{
  ...
  auto timestr_size = config_->must_get<ConfigKey::max_strlen>();
