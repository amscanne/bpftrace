          ...
          std::string_view expected)
{
  ...
  while (true) {
    ...
    if (input_done || output_done) {
      // Will fail: one is done, the other isn't.
      EXPECT_EQ(input, output);
      break;
    }
    if (input[input_index] != output[output_index]) {
      // Accept `->` and `.` interchangeably.
      if (input[input_index] == '-' && (input_index < input.size() - 1) &&
          input[input_index + 1] == '>' && output[output_index] == '.') {
        input_index += 2;
        output_index += 1;
        continue;
      }
      // The output may have more complete ';'. We can skip in this
      // case and defer to ensure the next line fails.
      if (output[output_index] == ';') {
        output_index++;
        continue;
      }
      // This will not be true, the strings are different.
      EXPECT_EQ(input, output);
      break;
    }
    input_index++;
