...
>>>>>>> 72046fd7 (inprog)
    ...
    TEST_F(clang_parser_btf, DISABLED_btf_arrays_multi_dim)
    {
      ...
      EXPECT_EQ(
          arrs->GetField("multi_dim").type.GetElementTy()->GetNumElements(), 2);
    ...
    {
      // size_t is defined in stddef.h, but if we have BTF, it should be
      // possible to extract it from there
      auto bpftrace = get_mock_bpftrace();
    ...
    TEST(clang_parser, struct_typedef)
    {
      ...
      auto max_align_struct =
          bpftrace.structs.Lookup("struct max_align_t").lock();
      ...
      EXPECT_TRUE(max_align_typedef->GetField("__clang_max_align_nonce1")
                      .type.IsIntTy());
      EXPECT_EQ(max_align_typedef->GetField("__clang_max_align_nonce1")
                    .type.GetSize(),
                8U);
      EXPECT_EQ(max_align_typedef->GetField("__clang_max_align_nonce1").offset,
                0);

      // double are not parsed correctly yet so these fields are junk for now
      EXPECT_TRUE(max_align_typedef->GetField("__clang_max_align_nonce2")
                      .type.IsNoneTy());
      EXPECT_EQ(max_align_typedef->GetField("__clang_max_align_nonce2")
                    .type.GetSize(),
                0U);
      EXPECT_EQ(max_align_typedef->GetField("__clang_max_align_nonce2").offset,
                16);
    }
